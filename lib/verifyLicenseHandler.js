...
function createVerifyLicenseHandler({ fetchImpl = fetch, env = process.env } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      if (!productId) {
        res.status(500).json({ error: 'Produit Gumroad non configuré côté serveur (GUMROAD_PRODUCT_ID).' });
        return;
      }
      ...
      const params = new URLSearchParams({ product_id: productId, license_key: licenseKey });
