...
async function runParser(pdfParseImpl, buffer) {
  try {
    ...
    // Journalisation volontairement limitée : jamais le contenu du PDF,
    // jamais de jeton, seulement de quoi diagnostiquer côté serveur.
    console.error('PDF extraction failed', { name: err?.name, message: err?.message, code: err?.code });
    ...
    wrapped.kind = looksMalformed ? 'malformed' : 'internal';
    throw wrapped;
  }
}
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      try {
        ...
        const status = wrapped.kind === 'malformed' ? 422 : 500;
