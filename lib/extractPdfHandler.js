...
function createExtractPdfHandler({ pdfParseImpl, env = process.env }) {
  return async function handler(req, res) {
    ...
    try {
      ...
      let parsed;
      try {
        parsed = await pdfParseImpl(buffer);
      } catch (parseErr) {
        console.error('Erreur pdf-parse:', parseErr);
        res.status(422).json({ error: 'Ce PDF est corrompu ou dans un format non supporté.' });
        return;
      }
...
module.exports = { createExtractPdfHandler, MAX_PDF_BYTES };
